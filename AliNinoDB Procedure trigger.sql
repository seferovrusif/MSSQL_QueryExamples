...
--	StockCount int,
--  ArticleCode nvarchar(20) not null unique,
...
-------------------------------------------------1. Create Procedure  -------------------------------------------------
--this procedure is for inserting values into different tables in one query.

Create PROCEDURE InsertValues(@AuthorName nvarchar(60),@AuthorSurname nvarchar(60),@BindingTitle nvarchar(60)
...
EXEC dbo.InsertValues @AuthorName ='Ali',@AuthorSurname='Safarov',@BindingTitle='Soft',@BookTitle='Sefiller',@BookDescription='good book',
@BookActualPrice =10,@BookDiscountPrice=9,@PublishingHouseTitle ='AA publish',@StockCount=100,@ArticleCode ='djsddr243f',@Pages =210,
@CategoryTitle='Books',@CategoryParentId=Null,@GenreTitle='Dram', @BookLanguageTitle='Eng'

---------I have given a separate procedure for adding multiple comments to one book
...
Exec dbo.InsertComment @Description='it was interesting',@BookArticleCode='djsddr243f',@Rating=4,@Name='Akif',@Email='Akif@gmail.com',@imgurl='aa.jpg'
...
if @BookTitle is not null
	UPDATE Books
	SET Title = @BookTitle                  
	WHERE  ArticleCode=@curentArticleCode   ----The current code is entered first, then the values to be changed
...
--if any value does not change, null is sent, non-null values are replaced by previous ones	
...
--To ensure that data is not lost during deletion, a corresponding column is created, it is indicated whether it is deleted, 
-- and the value of this column is changed instead each time it is deleted.
